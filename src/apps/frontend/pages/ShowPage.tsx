...
import { useJellyfinShow } from '../hooks/useJellyfin';
...
function ShowHero({ show, navigate, hero }: { show: Show; navigate: Navigate; hero?: HeroTweaks }) {
  ...
  const remaining = cont ? formatRemaining(cont.remaining) || cont.remaining || '' : '';
        ...
        <div style={{ marginTop: 28, display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          >
            ...
            <span style={{
              ...
              {inProgress
                ? (btnHover ? remaining : epLabel)
                : complete ? 'Visto' : `Reproducir ${label}`}
            </span>
          </button>
          <button style={{
