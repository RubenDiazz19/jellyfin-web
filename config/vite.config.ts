...
export default defineConfig(({ command, mode }) => ({
    ...
    resolve: {
        alias: [
            ...
            // Legacy code path aliases (moved to src/legacy/)
            { find: /^hooks\//, replacement: path.join(SRC_DIR, 'hooks') + '/' },
            { find: /^utils\//, replacement: path.join(SRC_DIR, 'utils') + '/' },
            { find: /^styles\//, replacement: path.join(SRC_DIR, 'styles') + '/' },
            { find: /^themes\//, replacement: path.join(SRC_DIR, 'themes') + '/' },
            { find: 'themes', replacement: path.join(SRC_DIR, 'themes') },
            { find: /^strings\//, replacement: path.join(SRC_DIR, 'strings') + '/' },
            { find: /^types\//, replacement: path.join(SRC_DIR, 'types') + '/' },
            { find: /^constants\//, replacement: path.join(SRC_DIR, 'constants') + '/' },
            { find: /^lib\//, replacement: path.join(SRC_DIR, 'legacy/lib') + '/' },
            { find: /^scripts\//, replacement: path.join(SRC_DIR, 'legacy/scripts') + '/' },
            { find: /^components\//, replacement: path.join(SRC_DIR, 'legacy/components') + '/' },
            { find: /^elements\//, replacement: path.join(SRC_DIR, 'legacy/elements') + '/' },
            // Root-level modules
            { find: 'RootAppRouter', replacement: path.join(SRC_DIR, 'RootAppRouter') },
    ...
    test: {
        ...
        setupFiles: [path.resolve(SRC_DIR, 'legacy/lib/globalize/vitest.setup.ts')],
        server: {
            deps: {
